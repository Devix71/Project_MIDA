...
def extract_patches(image, detections, label_of_interest):
    ...
    for idx, box in enumerate(boxes):
        if labels[idx] == label_of_interest:  # Assuming 'Flag' is represented by a specific label number
    ...
    for idx, box in enumerate(boxes):
        if scores[idx] >= detection_threshold:
            ...
            if label == 'Flag': 
                ...
                class_name += f" | Class {class_idx.item()} ({class_prob.item():.2f})"
...
def custom_draw(image, detections, classifications, original_size, new_size=(448, 448)):
    ...
    labels = detections[0]['labels']
    scores = detections[0]['scores']
    flag_label_index = 1  # Replace with the correct index for 'Flag'
    ...
    for idx, box in enumerate(boxes):
        if scores[idx] >= detection_threshold:
            ...
            classes = ['Map','Flag','Other']
            class_name = classes[label]
            flag_names = {0: 'Unknown', 1: 'Spanish', 2: 'French', 3: 'Russian', 4: 'Swedish'}

            classification_label = ""

            if label == flag_label_index and classifications:
                classification = classifications.pop(0)

                # Adjust the probability of the first class
...
# Load the checkpoint
checkpoint = torch.load('C:\\Users\\Devix\\OneDrive - University of Gothenburg\\FurhatProject\\Video_Skill\\camerafeed-demo\\object-detection-server\\best_object_detection_model.pth', map_location=torch.device('cpu'))
...
classification_model = torchvision.models.resnet50(pretrained=False, num_classes=5)   # Replace with your actual model class
classification_checkpoint = torch.load('object-detection-server/best_image_classification_model.pth', map_location=torch.device('cpu'))
classification_model.load_state_dict(classification_checkpoint)
classification_model.eval()


# Load configuration
with open('C:\\Users\\Devix\\OneDrive - University of Gothenburg\\FurhatProject\\Video_Skill\\camerafeed-demo\\object-detection-server\\launch.json') as f:
  config = json.load(f)
print(config)
...
def formatresult(detections, width, height):
    # Extract boxes, labels, and scores from the first (and only) element in detections
    boxes = detections[0]['boxes']
    labels = detections[0]['labels']
    scores = detections[0]['scores']
    classes = ['Map','Flag','Other']
...
def objectList(detections):
    ...
    for detection in detections:
        label = detection['item']  # 'item' key contains the class label
        output.append(label)
    return output
...
# Load configuration
with open('object-detection-server\launch.json') as f:
  config = json.load(f)
print(config)
